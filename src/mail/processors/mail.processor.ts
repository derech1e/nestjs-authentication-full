...
export class MailProcessor {
  ...
  public async confirmRegistration(
    job: Job<{ authentication: AuthenticationEntity; confirmUrl: string }>,
  ) {
    this._logger.log(
      `Sending confirm registration email to '${job.data.authentication.emailAddress}'`,
    );
    ...
    try {
      ...
      this._logger.error(
        `Failed to send confirmation email to '${job.data.authentication.emailAddress}'`,
      );
    }
  }
}
